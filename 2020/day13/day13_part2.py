...
def find_t(t, step, idx, busses):
    bus = busses[idx]
    seen = 0

    while True:
        if (t + bus[0]) % bus[1] == 0:
            if seen == 0 and idx != len(busses)-1:
                seen = t
            else:
                break
        t += step
    ...
    return find_t(t, step, idx, busses)


busses = []
for i, bus_id in enumerate(data[1].split(',')):
    if bus_id != 'x':
        bus_id = int(bus_id, 10)
        busses.append((i, bus_id))

bus0 = busses[0]

res = find_t(bus0[1], bus0[1], 1, busses)
print(f't = {res}')
