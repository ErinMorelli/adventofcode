...
class Monkey:
    ...
    def __init__(self, notes):
        ...
        # Create lambda for test
        test = int(lines[3].strip().split(' ')[-1])
        if_true = int(lines[4].strip().split(' ')[-1])
        if_false = int(lines[5].strip().split(' ')[-1])
        self.test_item = lambda x: if_true if (x % test) == 0 else if_false
    ...
    def take_turn(self):
        ...
        for item in self.items:
            worry = floor(self.inspect_item(item) / 3)
            monkey_id = self.test_item(worry)
            if monkey_id == self.id:
                to_keep.append(worry)
            else:
                to_throw.append((monkey_id, worry))
        self.items = to_keep
        return to_throw
    ...
    def __repr__(self):
        return f'<Monkey ID={self.id} items={self.items} inspected={self.inspected}>'
...
for i in range(20):
    for monkey in monkeys:
        for mid, new_item in monkey.take_turn():
