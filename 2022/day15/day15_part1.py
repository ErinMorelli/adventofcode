...
# Set to track spots without beacons
no_beacon = set()

# Iterate over possible y values
while x <= max_x+longest:
    coord = [x, y]
    # sensor, beacon, data
    for s, b, d in data:
        # if coord is a beacon, ignore
        if coord == b:
            continue
        # if this coord is a sensor add to list
        if coord == s:
            no_beacon.add(str(coord))
            continue
        # Calc distance between coord and sensor
        dist = distance(s, coord)
        # If distance is <= distance between sensor and beacon, add to list
        if dist <= d:
            no_beacon.add(str(coord))
    x += 1
