...
data = [[x[:len(x)//2], x[len(x)//2:]] for x in raw_data]


def priority(val):
    if val in string.ascii_lowercase:
        return string.ascii_lowercase.index(val) + 1
    else:
        return string.ascii_uppercase.index(val) + 27
...
for sack in data:
    comp1 = set(sack[0])
    comp2 = set(sack[1])
    overlap = comp1.intersection(comp2).pop()
