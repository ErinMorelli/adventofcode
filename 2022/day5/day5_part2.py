...
for row in rows:
    ...
    for i, (_, result) in enumerate(results):
        if result != '':
            stacks[i].append(result)

for move in raw_moves.strip().split('\n'):
    result = re.match(r'move (\d+) from (\d+) to (\d+)', move)
    amt, from_col, to_col = result.groups()
    stack1 = stacks[int(from_col) - 1]
    conts = stack1[:int(amt)]
    stacks[int(from_col) - 1] = stack1[int(amt):]
    stack2 = stacks[int(to_col) - 1]
    stacks[int(to_col) - 1] = conts + stack2
