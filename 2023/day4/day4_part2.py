...
for idx, line in enumerate(raw_data):
    [raw_winners, raw_yours] = line.split(':')[1].split('|')
    winners = set([int(x.strip()) for x in re.split(r'\s+', raw_winners.strip())])
    yours = set([int(x.strip()) for x in re.split(r'\s+', raw_yours.strip())])
    overlap = winners.intersection(yours)
    all_cards[idx+1] = len(overlap)


def play_card(card_id, card):
    count = 1
    if card > 0:
        for winner_id in range(card_id + 1, card_id + 1 + card):
            count += play_card(winner_id, all_cards[winner_id])
    return count


for card_id, card in all_cards.items():
    card_count += play_card(card_id, card)
