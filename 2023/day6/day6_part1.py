...
expenses-paid trip to Desert Island!" That must be where the sand comes from!
...
times = [int(x) for x in split(r'\s+', raw_time)[1:]]
distances = [int(x) for x in split(r'\s+', raw_distance)[1:]]

win_counts = []

for idx, race_time in enumerate(times):
    race_dist = distances[idx]
    possible_wins = 0

    for ms in range(0, race_time+1):
        mm = (race_time - ms) * ms
        if mm > race_dist:
            possible_wins += 1

    win_counts.append(possible_wins)

print(reduce(lambda a, b: a*b, win_counts))
