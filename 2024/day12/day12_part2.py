...
def check_plot(x, y, val):
    ...
    # up
    if y - 1 >= 0 and data[y - 1][x] == val:
        a, ps = check_plot(x, y - 1, val)
        area += a
        plots.update(ps)

    # down
    if y + 1 < max_y and data[y + 1][x] == val:
        a, ps = check_plot(x, y + 1, val)
        area += a
        plots.update(ps)

    # left
    if x - 1 >= 0 and data[y][x - 1] == val:
        a, ps = check_plot(x - 1, y, val)
        area += a
        plots.update(ps)

    # right
    if x + 1 < max_x and data[y][x + 1] == val:
        a, ps = check_plot(x + 1, y, val)
        area += a
        plots.update(ps)
...
for y, row in enumerate(data):
    for x, plot in enumerate(row):
        ...
        a, ps = check_plot(x, y, plot)
        all_plots.append([plot, a, ps])

prices = list()
for plot in list(all_plots):
    val, area, cx = plot
    coords = list(cx)

    xs = list(set(i[0] for i in coords))
    ys = list(set(i[1] for i in coords))
    ...
    for rowy in ys:
        row = sorted([c for c in coords if c[1] == rowy], key=lambda i: i[0])

        for i, (x, y) in enumerate(row):
            up =  y - 1 < 0 or data[y - 1][x] != val
            up_right = y - 1 < 0 or x + 1 >= max_x or data[y - 1][x + 1] != val
            right = x + 1 >= max_x or data[y][x + 1] != val
            down_right = y + 1 >= max_y or x + 1 >= max_x or data[y + 1][x + 1] != val
            down = y + 1 >= max_y or data[y + 1][x] != val

            if up and (right == up_right or (right and not up_right)):
                tops += 1

            if down and (right == down_right or (right and not down_right)):
                bottoms += 1

    for colx in xs:
        col = sorted([c for c in coords if c[0] == colx], key=lambda i: i[1])

        for i, (x, y) in enumerate(col):
            left = x - 1 < 0 or data[y][x - 1] != val
            left_down = x - 1 < 0 or y + 1 >= max_y or data[y + 1][x - 1] != val
            down = y + 1 >= max_y or data[y + 1][x] != val
            right_down = x + 1 >= max_x or y + 1 >= max_y or data[y + 1][x + 1] != val
            right = x + 1 >= max_x or data[y][x + 1] != val

            if left and (down == left_down or (down and not left_down)):
                lefts += 1

            if right and (down == right_down or (down and not right_down)):
                rights += 1

    sides = tops + bottoms + lefts + rights
    price = area * sides
    prices.append(price)
