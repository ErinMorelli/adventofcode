...
for chunk in raw_data.split('\n\n'):
    [a, b, p] = chunk.splitlines()

    ax, ay = map(int, re.findall(r'[XY][+=](\d+)', a))
    bx, by = map(int, re.findall(r'[XY][+=](\d+)', b))
    px, py = map(int, re.findall(r'[XY][+=](\d+)', p))
    ...
    if (
        a_press <= 100 and
        b_press <= 100 and
        a_press.is_integer() and
        b_press.is_integer()
    ):
        tokens += int((a_press * a_val) + (b_press * b_val))
